...
def fetch_codelist(name, url, lang="en"):
    ...
    output.append(["Category", "Code", "Title_" + lang,
                   "Description_" + lang, "Source"])
    for row in csvData:
        output.append([row.get("Category"), row.get("Code"), row.get(
            "Title_" + lang), row.get("Description_" + lang), row.get("URL", "")])
...
if __name__ == "__main__":
    ...
    r = requests.get(
        "https://spreadsheets.google.com/feeds/worksheets/1FEJr2A1pR2DnKCw24lj78TPBFs1HH0tiZmRnYhmwUFc/public/basic")
    ...
    iati = requests.get(
        "http://iatistandard.org/codelists/downloads/clv2/csv/en/OrganisationRegistrationAgency.csv")
